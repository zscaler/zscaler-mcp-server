...
.. list-table::
   ...
   * - Integration
     ...
     - Best for
   * - **Claude Code Plugin**
     - `Claude Code <https://docs.anthropic.com/en/docs/claude-code>`__ CLI
...
The plugin bundles service-specific skills that Claude auto-activates based on your prompt. See the :doc:`Skills catalog <../../skills/index>` for the full list.
...
- `Claude Code <https://docs.anthropic.com/en/docs/claude-code>`__ installed
- `uv <https://docs.astral.sh/uv/>`__ installed (provides the ``uvx`` runner used by the plugin's ``.mcp.json``)
...
.. image:: /_static/img/claude-extension-install01.png
   :alt: Directory search for Zscaler MCP Server
   :align: center
...
.. image:: /_static/img/claude-extension-install02.png
   :alt: Zscaler MCP Server extension details with All requirements met
   :align: center
...
.. image:: /_static/img/claude-extension-install03.png
   :alt: Install confirmation dialog
   :align: center
...
.. image:: /_static/img/claude-extension-install04.png
   :alt: Extension enabled, with Configure button
   :align: center
...
.. image:: /_static/img/claude-extension-install05.png
   :alt: Configuration form with Zscaler credential fields
   :align: center
...
.. list-table::
   ...
   * - ``ZSCALER_VANITY_DOMAIN``
     - ZIdentity vanity domain (e.g. ``acme.zsapi.net``)
   * - ``ZSCALER_CLOUD``
     - Cloud override; leave ``production`` unless you're on a non-prod cloud
   * - **Enabled Tools**
     - Optional comma-separated allowlist; leave empty to expose every read-only tool the server registers
   * - **User-Agent comment**
     - Optional suffix appended to outbound API calls' ``User-Agent`` header — useful for tagging traffic in audit logs
...
- `Main README <https://github.com/zscaler/zscaler-mcp-server/blob/master/README.md>`__
- :doc:`../../guides/claude-desktop-extension`
- :doc:`../../security/write-operations`
- :doc:`../../skills/index`
