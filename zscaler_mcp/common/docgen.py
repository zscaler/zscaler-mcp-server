...
def _service_anchor(service: str) -> str:
    ...
    Mirrors the behaviour of ``github-slugger`` (which both GitHub's
    markdown renderer and Docusaurus use for heading IDs): lowercase the
    text, drop punctuation that isn't a space or hyphen, then turn each
    remaining space into a single hyphen — without collapsing runs of
    spaces first. For a heading like ``ZIA — Internet Access`` the
    em-dash is dropped, leaving two spaces, which become a double-dash:
    ``zia--internet-access``. Both renderers agree on that slug, so the
    auto-generated TOC links keep working on both surfaces.
    """
...
def _render_toolset_catalog_json(inv: Inventory, catalog: ToolsetCatalog) -> str:
    """Catalog as a structured JSON document for the docs-site React UI.

    The docs-site `ToolsetsCatalog` component prefers a richer data shape
    than the markdown tables can express — service grouping, per-card
    "default" badge, and tool counts. Writing JSON here keeps the source
    of truth in `zscaler_mcp/common/toolsets.py` while letting the UI
    render whatever it wants without scraping markdown.

    Schema (stable contract — bump a version field here if it ever
    breaks):
    ...
    # ``meta`` always renders first; everything else follows the
    # canonical service ordering used by the markdown renderers.
    service_order = [
        "meta",
        "zia",
        "zpa",
    ...
    for svc in service_order:
        ...
        services_out.append(
            {
                "code": svc.upper(),
                "label": _service_short_label(svc),
                "description": _service_friendly_subtitle(svc),
                "toolsets": [
                    {
                        "id": tsid,
                        "description": catalog.get(tsid).description.strip(),
                        "default": bool(catalog.get(tsid).default),
                        "tool_count": tool_count(tsid),
                        "always_on": svc == "meta",
                    }
    ...
    payload = {
        "_generated": (
            "Generated by zscaler_mcp/common/docgen.py — do not edit by hand. "
            "Run `make generate-docs` after changing toolset metadata."
        ),
        "services": services_out,
    }

    # Stable, deterministic formatting so check-docs can byte-compare.
    return json.dumps(payload, indent=2, ensure_ascii=False) + "\n"


def _service_short_label(service: str) -> str:
    """Short, uppercase code shown in the card-grid section header.

    Mirrors the labels in the screenshots: "META", "ZIA", "ZPA",
    "ZDX", "ZCC", "ZTW", "ZID", "EASM", "ZINS", "ZMS".
    """
    return {
        "meta": "META",
        "zia": "ZIA",
...
def _service_friendly_subtitle(service: str) -> str:
    """The pill text next to the short label (e.g. "Internet Access")."""
    return {
        "meta": "cross-service",
        "zia": "Internet Access",
        "zpa": "Private Access",
...
#
# When ``region`` is a string, the renderer's output replaces the body
# between matching ``<!-- generated:start <region> -->`` / end markers
# in a hand-edited Markdown file (the original use case).
#
# When ``region`` is ``None``, the renderer's output IS the entire file
# content. This is how non-Markdown auto-generated files (e.g. the JSON
# catalog the docs-site React UI consumes) are kept in sync — the file
# is fully owned by the generator and any manual edit gets stomped on
# the next ``make generate-docs``.
TARGETS: List[Tuple[str, Optional[str], Callable[[Inventory, ToolsetCatalog], str]]] = [
    (
        "docs/guides/supported-tools.md",
        "tools",
        _render_supported_tools_region,
    ),
    # Docusaurus site mirror — kept in lock-step with docs/guides/* so the
    # public site never lags. Same renderer, same region marker, separate
    # file on disk. If the mirror falls out of sync, `--check-docs` flags it.
    (
        "docs-site/docs/guides/supported-tools.md",
        "tools",
        _render_supported_tools_region,
    ),
    (
    ...
    # NOTE: The docs-site copy of the toolsets page (`docs-site/docs/
    # guides/toolsets.mdx`) no longer renders the markdown table. It
    # uses the `<ToolsetsCatalog />` React component which consumes the
    # JSON target below. Removing this auto-region target keeps the
    # `.mdx` file editable and prevents docgen from trying to rewrite
    # markers that no longer exist there.
    #
    # Whole-file target — the React ToolsetsCatalog component imports
    # this JSON to render the card grid. No markers (region=None).
    (
        "docs-site/src/data/toolsets.json",
        None,
        _render_toolset_catalog_json,
    ),
...
def _compute_updated_content(
    path: Path,
    region: Optional[str],
    renderer: Callable[[Inventory, ToolsetCatalog], str],
    inv: Inventory,
) -> Tuple[str, str]:
    ...
    * ``region`` is set — Markdown region rewrite. Caller's file must
      exist and contain the matching marker pair.
    * ``region`` is ``None`` — whole-file replacement. If the file
      doesn't exist yet (first-ever generation), ``original`` is
      returned as the empty string so ``updated != original`` triggers
      a write.
    """
    body = renderer(inv, TOOLSETS)
