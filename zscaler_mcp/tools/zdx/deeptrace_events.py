...
def zdx_get_deeptrace_events(
    ...
    """
    Get events from a ZDX deep trace session.
    This is a read-only operation.

    Returns event metrics trend for a device during the deep trace session.
    Events include Zscaler, hardware, software, and network event changes.
    Useful for identifying configuration or environmental changes that may
    ...
    Raises:
        Exception: If the events retrieval fails due to API errors.

    Examples:
        Get events from a deep trace:
        >>> events = zdx_get_deeptrace_events(
        ...     device_id="device123",
        ...     trace_id="trace456"
        ... )
    """
    ...
    result, _, err = client.zdx.troubleshooting.get_deeptrace_events(device_id, trace_id)
    if err:
        raise Exception(f"Events retrieval failed: {err}")
