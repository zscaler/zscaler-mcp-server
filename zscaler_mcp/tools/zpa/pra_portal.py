...
def pra_portal_manager(
    ...
    portal_id: Annotated[
        str, Field(description="Portal ID for read, update, or delete operations.")
    ] = None,
    name: Annotated[str, Field(description="Name of the PRA portal.")] = None,
    description: Annotated[
        str, Field(description="Description of the PRA portal.")
    ] = None,
    enabled: Annotated[
        bool, Field(description="Whether the portal is enabled.")
    ] = True,
    domain: Annotated[
        str, Field(description="Domain for the portal (required for create/update).")
    ] = None,
    certificate_id: Annotated[
        str,
        Field(
            description="Certificate ID (required when creating or updating a portal)."
        ),
    ] = None,
    user_notification: Annotated[
        str, Field(description="User notification message for the portal.")
    ] = None,
    user_notification_enabled: Annotated[
        bool, Field(description="Whether user notifications are enabled.")
    ] = None,
    ...
    if action == "create":
        if not all([name, domain]):
            raise ValueError(
                "Both 'name' and 'domain' are required for portal creation"
            )
        ...
        if not certificate_id:
            ...
            raise ValueError(
                "name, domain, and certificate_id are required for portal creation"
            )
