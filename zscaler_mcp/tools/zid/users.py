...
def zid_search_users(
    name: Annotated[
        str,
        Field(
            description="User name, login name, or email to search for (case-insensitive partial match)."
        ),
    ],
    query_params: Annotated[
...
def zid_get_user_groups(
    user_id: Annotated[str, Field(description="User ID.")],
    query_params: Annotated[
        Optional[Dict], Field(description="Optional filters: offset, limit.")
    ] = None,
    service: Annotated[str, Field(description="The service to use.")] = "zid",
) -> List[Dict]:
...
def zid_get_user_groups_by_name(
