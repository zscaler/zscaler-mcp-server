...
def zia_dlp_engine_manager(
    ...
    Args:
        ...
        engine_id (int/str, optional): Required for get operation.
    ...
    Examples:
        ...
        >>> engines = zia_dlp_engine_manager(action="list")
        
        List engines with name and ID only (faster):
        >>> engines = zia_dlp_engine_manager(action="list_lite")
        
        Search for engines containing "credit card":
        >>> engines = zia_dlp_engine_manager(action="list", search="credit card")
        
        Get a specific engine by ID:
        >>> engine = zia_dlp_engine_manager(action="get", engine_id="12345")
        
        Search for engines containing "SSN":
        >>> engines = zia_dlp_engine_manager(action="list", search="SSN")
    
    Note:
        - The list action returns full engine details including expressions and configuration.
