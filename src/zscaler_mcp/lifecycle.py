...
def _do_soft_reload(dotenv_path: Optional[str]) -> None:
    ...
    * Re-applies env-driven toggles that are read once at startup
    ...
    try:
        from zscaler_mcp.security import refresh_tool_call_logging
...
def _default_dotenv_candidates() -> list[Path]:
    ...
    v2 note: the package lives under ``src/zscaler_mcp/`` (src layout), so the
    project root is two levels up from this module's directory, not one.
    """
...
def _current_version() -> str:
    ...
    from zscaler_mcp.common.utils import _get_package_version
