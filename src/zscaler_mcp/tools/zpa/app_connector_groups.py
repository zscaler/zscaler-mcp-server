...
class ConnectorGroupSummary(AgentView):
    """Lean view — identify and reason about an app connector group."""

    id: str = Field(description="Connector group ID. Use this in follow-up calls.")
    ...
    connector_count: int = Field(description="Number of member connectors (relational signal).")


class ConnectorGroupDetail(ConnectorGroupSummary):
    """Full view — summary plus relational + config + provenance fields."""

    connector_ids: list[str] = Field(default_factory=list, description="IDs of member connectors.")
    server_group_ids: list[str] = Field(
        default_factory=list, description="IDs of associated server groups."
    )
    version_profile: Optional[str] = Field(default=None, description="Version profile.")
    dns_query_type: Optional[str] = Field(default=None, description="DNS query type.")
...
def _connectors(raw: dict[str, Any]) -> list[Any]:
    return coalesce(raw, "connectors", "connector_ids", "connectorIds")


def _server_groups(raw: dict[str, Any]) -> list[Any]:
    return coalesce(raw, "server_groups", "serverGroups", "server_group_ids")
...
def _shape_summary(raw: dict[str, Any]) -> ConnectorGroupSummary:
    return ConnectorGroupSummary(
        ...
        connector_count=len(_connectors(raw)),
    )


def _shape_detail(raw: dict[str, Any]) -> ConnectorGroupDetail:
    connectors = _connectors(raw)
    sgs = _server_groups(raw)
    return ConnectorGroupDetail(
        id=str(pick(raw, "id", default="")),
        name=pick(raw, "name", default=""),
        enabled=bool(pick(raw, "enabled", default=False)),
        description=pick(raw, "description"),
        ...
        connector_ids=_ids(connectors),
        server_group_ids=_ids(sgs),
        version_profile=pick(raw, "version_profile", "versionProfile"),
        dns_query_type=pick(raw, "dns_query_type", "dnsQueryType"),
...
@tool(
    ...
    output_view=ConnectorGroupSummary,
    is_list=True,
)
def zpa_list_app_connector_groups(args: ListConnectorGroupsInput) -> list[dict[str, Any]]:
    """List ZPA app connector groups as curated, agent-facing views (read-only)."""
...
@tool(
    ...
    output_view=ConnectorGroupDetail,
    is_list=False,
)
def zpa_get_app_connector_group(args: GetConnectorGroupInput) -> dict[str, Any]:
    """Get one ZPA app connector group as a curated, agent-facing view (read-only)."""
