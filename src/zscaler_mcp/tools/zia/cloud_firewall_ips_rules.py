"""ZIA Cloud Firewall IPS rules — list, get, create, update, delete.

Mirrors v1's ``client.zia.cloud_firewall_ips`` SDK calls. Common rule fields are typed; the
...
_ADVANCED_DESC = (
    ...
    "IPS-specific fields (threat_categories, dest_ip_categories), and any other SDK-supported field. List values may be JSON strings."
)

ACTION_DESC = "IPS rule action: ALLOW, BLOCK_DROP, BLOCK_RESET, etc."
...
def zia_list_cloud_firewall_ips_rules(args: ListInput) -> list[dict[str, Any]]:
    """List ZIA Cloud Firewall IPS rules as curated summaries."""
    client = get_zscaler_client(service="zia")
    qp = {"search": args.search} if args.search else {}
    rules, _, err = client.zia.cloud_firewall_ips.list_rules(query_params=qp)
    if err:
        raise RuntimeError(f"Failed to list Cloud Firewall IPS rules: {err}")
...
def zia_get_cloud_firewall_ips_rule(args: GetInput) -> dict[str, Any]:
    """Get a single ZIA Cloud Firewall IPS rule by ID with member references."""
    client = get_zscaler_client(service="zia")
    rule, _, err = client.zia.cloud_firewall_ips.get_rule(args.rule_id)
    if err:
        raise RuntimeError(f"Failed to get Cloud Firewall IPS rule {args.rule_id}: {err}")
...
def zia_create_cloud_firewall_ips_rule(args: CreateInput) -> dict[str, Any]:
    """Create a ZIA Cloud Firewall IPS rule (write). Activate after."""
    ...
    client = get_zscaler_client(service="zia")
    rule, _, err = client.zia.cloud_firewall_ips.add_rule(**payload)
    if err:
        raise RuntimeError(f"Failed to create Cloud Firewall IPS rule: {err}")
    return shape_rule_detail(rule.as_dict()).model_dump()
...
def zia_update_cloud_firewall_ips_rule(args: UpdateInput) -> dict[str, Any]:
    """Update a ZIA Cloud Firewall IPS rule (write, PUT-replace). Activate after."""
    payload = build_rule_payload(
        scalars={
            "name": args.name,
            "action": args.action,
    ...
    client = get_zscaler_client(service="zia")
    rule, _, err = client.zia.cloud_firewall_ips.update_rule(args.rule_id, **payload)
    if err:
        raise RuntimeError(f"Failed to update Cloud Firewall IPS rule {args.rule_id}: {err}")
    return shape_rule_detail(rule.as_dict()).model_dump()
...
def zia_delete_cloud_firewall_ips_rule(args: DeleteInput) -> dict[str, Any]:
    """Delete a ZIA Cloud Firewall IPS rule (destructive). Activate after."""
    client = get_zscaler_client(service="zia")
    _, _, err = client.zia.cloud_firewall_ips.delete_rule(args.rule_id)
    if err:
        raise RuntimeError(f"Failed to delete Cloud Firewall IPS rule {args.rule_id}: {err}")
    return OperationResult(
        success=True, message=f"Cloud Firewall IPS rule {args.rule_id} deleted successfully."
    ).model_dump()
