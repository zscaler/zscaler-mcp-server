"""GCP Secret Manager credential loader for the Zscaler MCP server (v2).

Ported verbatim from v1 (``zscaler_mcp/cloud/gcp_secrets.py``) — the module has
no product-specific dependencies. When the server runs on GCP (Cloud Run, GKE,
Compute Engine), this fetches Zscaler API credentials from GCP Secret Manager at
startup and injects them as environment variables before the SDK clients are
created (which v2 does lazily, on the first tool call).
...
Naming convention:
    Env var names map to Secret Manager IDs by lowercasing and replacing
    underscores with hyphens::

        ZSCALER_CLIENT_ID      ->  zscaler-client-id
        ZSCALER_CLIENT_SECRET  ->  zscaler-client-secret

    Secrets that don't exist are silently skipped (the env var keeps whatever
    value it already has). Requires the optional ``gcp`` extra:
...
def load_secrets() -> None:
    """Fetch credentials from GCP Secret Manager and set them as env vars.

    No-op unless ``ZSCALER_MCP_GCP_SECRET_MANAGER`` is truthy. Raises
    ``SystemExit`` on fatal errors so the server stops cleanly.
    """
    if not is_enabled():
        return
