...
def access_policy_manager(
    ...
    - delete: rule_id
    - list/get: policy_type is inferred as 'access'
    """
    ...
    if action == "create":
        if not all([name, action_type]):
            raise ValueError("'name' and 'action_type' are required for creating an access rule")
        ...
        if not rule_id:
            raise ValueError("'rule_id' is required for updating an access rule")
        ...
        if not rule_id:
            raise ValueError("'rule_id' is required for deleting an access rule")
