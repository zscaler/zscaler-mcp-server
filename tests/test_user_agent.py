...
class TestUserAgent(unittest.TestCase):
    ...
    def test_user_agent_format_without_comment(self):
        ...
        self.assertTrue(
            parts[0].startswith("zscaler-mcp-server/"), f"Invalid server part: {parts[0]}"
        )
        self.assertTrue(parts[1].startswith("python/"), f"Invalid python part: {parts[1]}")
    ...
    def test_user_agent_format_with_comment(self):
        ...
        self.assertTrue(
            user_agent.endswith(comment), f"Comment not found in user agent: {user_agent}"
        )
    ...
    def test_user_agent_contains_expected_components(self):
        ...
        self.assertIn(
            f"python/{python_version}", user_agent, f"Python version not found in: {user_agent}"
        )
        self.assertIn(f"{system}/{machine}", user_agent, f"OS/arch not found in: {user_agent}")
        self.assertIn("zscaler-mcp-server/", user_agent, f"Server name not found in: {user_agent}")
