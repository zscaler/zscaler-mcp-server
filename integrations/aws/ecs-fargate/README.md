...
5. **MCP auth mode** — `zscaler` (default, HTTP Basic with OneAPI creds), `jwt`, `api-key`, `oidc`, or `none`
...
| `oidc` | `Authorization: Bearer <jwt>` from an OAuth 2.1 browser login | JWKS lookup against the IdP's discovery document | An IdP you control (Auth0, Cognito, Entra ID, ...). Set `OIDCPROXY_CONFIG_URL`, `OIDCPROXY_CLIENT_ID` and `OIDCPROXY_BASE_URL`. No client secret. |
