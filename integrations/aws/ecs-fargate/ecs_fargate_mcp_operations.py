#!/usr/bin/env python3
"""Zscaler MCP Server — AWS ECS-Fargate deployment & lifecycle.

Sibling of ``integrations/aws/bedrock-agentcore/aws_mcp_operations.py``.
Where that script deploys the MCP server *inside* AWS Bedrock AgentCore
(SigV4-protected, only reachable from AWS-side agents), **this** script
deploys it as a standard public-internet HTTPS service on ECS-Fargate,
fronted by an Application Load Balancer with an ACM TLS certificate —
exactly the topology any MCP client (Claude Desktop, Cursor, Gemini CLI,
VS Code, Windsurf, GitHub Copilot CLI, plus any future MCP client) can
talk to directly.
...
Topology::
    ...
    User / MCP client  ─────►  ALB (TLS @ 443)
                                   │
                                   ▼
                            ECS-Fargate task  ──►  Zscaler OneAPI
                            (Docker Hub image)

Commands:
    deploy      Build asset bucket, upload nested templates, launch root stack
    status      Show stack status, MCP URL, ALB DNS, service health
...
Manager and the task definition mounts them from there.
"""
...
_TAGLINE = "AWS ECS-Fargate Deployment"
...
NESTED_TEMPLATES = ["network.yaml", "iam.yaml", "secrets.yaml", "ecs.yaml"]
ROOT_TEMPLATE = "zscaler-mcp-root.yaml"

DEFAULT_REGION = "us-east-1"
DEFAULT_STACK = "zscaler-mcp-ecs-fargate"
DEFAULT_PREFIX = "zscaler-mcp"
DEFAULT_IMAGE = "zscaler/zscaler-mcp-server:latest"
DEFAULT_SERVER_NAME = "zscaler-mcp-aws-ecs"
...
def _apply_state_for_redeploy(state: dict[str, Any], env: dict[str, str]) -> None:
    """Pre-populate ``env`` from the saved state so a re-deploy is friction-free.

    When a state file from a prior successful (or partially-successful) deploy
    is found, this helper merges the saved values into ``env`` BEFORE the
    ``resolve_*`` helpers run. The result: re-running ``deploy`` against an
    existing stack just rolls out whatever changed in the operator's ``.env``
    (image tag, feature flags, auth mode, write-tool allowlist) without
    re-prompting for every dimension of the original deploy.

    Three categories of values, each handled differently:

    1. **Locked** (CFN cannot change these between deploys without a destroy):
       region, stack name, prefix, network mode. Saved values WIN over ``.env``
       — overriding them in ``.env`` after the first deploy would either fail
       (region/stack-name/prefix mismatch with the existing stack) or trigger
       a destructive operation (network-mode swap deletes the VPC). Forcing
       the state value here means the prompt shows the only legal answer and
       the operator just hits Enter.

    2. **Forced UseExisting** (resource already provisioned, don't re-create):
       credential source. The first deploy may have used ``CredentialSource=
       CreateNew`` and CFN provisioned the Secrets Manager secret. On
       re-deploy we always pin to ``UseExisting`` pointing at the saved
       ``secret_arn`` — CFN keeps managing the same secret, the secret's
       contents stay untouched, and the operator avoids a second prompt for
       sensitive credentials. To rotate the secret's contents the operator
       updates it directly in AWS Secrets Manager (out of band).

    3. **Soft defaults** (operator can still override via ``.env``):
       TLS mode, auth mode, image URI, server name. These are valid things
       to change between deploys (e.g. new image tag for a rolling update,
       flip auth-mode none→jwt). ``env.setdefault`` lets ``.env`` win.
    """
    # Locked dimensions
    if state.get("region"):
    ...
    if state.get("network_mode"):
        ...
        # For UseExisting networks, the VPC/subnet IDs come from .env on the
        # original deploy — leave those alone and let resolve_network read
        # them. For CreateNew, CFN already owns the VPC so no IDs are needed.

    # Forced UseExisting for credentials
    if state.get("secret_arn"):
        env["ZSCALER_SECRET_NAME"] = state["secret_arn"]
        # resolve_credentials() switches to UseExisting whenever
        # ZSCALER_SECRET_NAME is non-empty.
...
_ROLLBACK_STATES = (
    ...
    "DELETE_IN_PROGRESS",  # CFN sweeps the stack when OnFailure=DELETE fires
)
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if "does not exist" in str(e):
                ...
                # CREATE/UPDATE: stack vanished mid-flight. Almost always
                # means OnFailure=DELETE swept it away after a failure;
                # surface whatever reasons we managed to capture.
                err(
        ...
        # Snapshot failure reasons the FIRST time we see a rollback/delete
        # state — once CFN finishes deleting the stack they're unreachable.
        if (
            ...
            try:
                ...
                pass  # stack may already be gone — fall through
...
def _check_image_compatibility(
    ...
    *,
    ...
    """Pre-flight: catch architecture mismatches before submitting the stack.

    Fargate's image-pull layer obeys the OCI/Docker manifest list and picks
    the right architecture automatically — but ONLY if the image is actually
    multi-arch. The single biggest footgun for operators building custom
    images on an M-series Mac is::

        docker build -t my-repo/zscaler-mcp:dev .
        docker push  my-repo/zscaler-mcp:dev

    With no ``--platform`` flag, Docker produces a single-arch ``arm64`` image.
    Fargate (which defaults to ``LINUX/X86_64``) pulls it, tries to ``exec``
    the binary, and the kernel rejects it with::

        exec /usr/local/bin/python: exec format error

    The task exits 255, the ECS service circuit-breaker trips, the CFN stack
    fails after ~10 minutes, and the operator gets a cryptic ``ServiceTls``
    failure.  This pre-flight catches the situation in under a second by
    inspecting the ECR image manifest BEFORE we start the deploy.

    Marketplace constraint: we do NOT inject ``RuntimePlatform`` into the task
    definition — that would lock the deployment to one CPU at the template
    level, which the AWS Marketplace task-definition contract does not allow.
    The right fix is always "publish a multi-arch image" or "build for amd64".

    Scope:
      * ECR images → inspected via the ECR API (no docker pull required).
      * Public Docker Hub / ECR Public / other registries → skipped silently
        (we'd need ambient docker auth and that's not portable). The official
        ``zscaler/zscaler-mcp-server:latest`` image is published multi-arch
        so this is a non-issue in the default case.
    """
    m = _ECR_URI_RE.match(image_uri.strip())
    if not m:
        return  # not ECR — out of scope
    ...
    try:
        ...
        # IAM doesn't allow ECR inspection from this principal, or the repo
        # is in another account we can't see. Skip silently — we don't want
        # to block a deploy on a precaution.
        warn(
            f"Skipping image-arch pre-flight ({exc.response.get('Error', {}).get('Code', '')}). "
            f"If your ECR image is single-arch, ensure it was built with "
            f"`docker buildx build --platform linux/amd64 --push`."
        )
        return
    ...
    if not images:
        warn(
            f"Image {image_uri} not found in ECR. The deploy will fail to "
            "pull it — push the image first, then re-run."
        )
        return
    ...
    # ─ Multi-arch (manifest list / OCI index): trust Fargate to pick a variant
    if "manifest.list" in media_type or "image.index" in media_type or "manifests" in manifest:
        ...
        warn(
            ...
            "but does NOT include amd64 — Fargate defaults to LINUX/X86_64 "
            "and will fail to pull. Add an amd64 variant before deploying."
        )
        if not non_interactive and not prompt_bool("Continue anyway?", default=False):
            die("Aborted by user.")
        return

    # ─ Single-arch: arch lives on the config blob, not the manifest
    ...
    if not arch:
        return
    if arch == "amd64":
        info(f"  Image {image_uri} is single-arch amd64 — compatible with Fargate.")
        return

    # The actual failure mode you just hit.
    err(
        f"Image {image_uri} is single-arch {arch}-only.\n"
        f"  Fargate defaults to LINUX/X86_64. Pulling an {arch} image on "
        f"x86 produces:\n"
        f"      exec /usr/local/bin/python: exec format error\n"
        f"  and the task exits with code 255 within seconds.\n\n"
        f"  Fixes (pick one):\n"
        f"    1. Use the public multi-arch image (recommended):\n"
        f"         unset ZSCALER_MCP_IMAGE_URI in .env\n"
        f"    2. Rebuild and push as multi-arch from your Mac:\n"
        f'         docker buildx build --platform linux/amd64,linux/arm64 \\\n'
        f"             -t {image_uri} --push .\n"
        f"    3. Rebuild and push as amd64-only:\n"
...
def _dump_recent_container_logs(
    sess: boto3.Session,
    log_group: str,
    *,
    streams: int = 3,
    ...
    """Surface the container's stdout/stderr inline on deploy failure.

    Exit code 255 on ECS almost always means the container started but
    immediately exited because the application threw an unhandled
    exception. The CFN ``ServiceTls`` resource failing tells us the
    deployment circuit-breaker tripped, but not WHY — the Python stack
    trace is in CloudWatch Logs. Dumping it inline saves a 5-minute
    round-trip through the console / `aws logs tail`.

    Best-effort: silent no-op when the log group doesn't exist (the
    container never got far enough to write logs) or any AWS API call
    fails. We never want this helper to mask the original failure.
    """
    try:
        ...
        if not stream_names:
            info(
                f"No log streams yet in {log_group} — the container may "
                "have crashed before stdout was flushed."
            )
            return
        print()
        info(f"Last container logs from {log_group}:")
        print("  " + "─" * 72)
        ...
        if code in ("ResourceNotFoundException", "LogGroupNotFound"):
            info(
                f"Log group {log_group} does not exist — the container "
                "likely never started."
            )
        else:
            warn(f"Could not fetch container logs ({code}): {exc}")
    except Exception as exc:  # noqa: BLE001
        warn(f"Could not fetch container logs: {exc}")
...
def _build_mcp_remote_args(url: str, headers: dict[str, str]) -> list[str]:
    ...
    # the wire in cleartext. The Fargate deployment's `TlsMode=None` path
...
def resolve_credentials(
    ...
    For the ``UseExisting`` branch we have to resolve the secret's full ARN
    (including the 6-char random suffix AWS appends). ECS task definitions
    reject ``arn:...:secret:<name>-*`` and ``arn:...:secret:<name>`` —
    they need the exact ARN. We accept either a friendly name (looked up
    via ``DescribeSecret``) or a pre-baked ARN.
    """
    ...
    if not non_interactive:
        ...
        if use_existing:
            existing = prompt(
                "Existing Secrets Manager secret name or ARN "
                "(e.g. zscaler/mcp/credentials)",
                default=existing or None,
            )

    if use_existing:
        # Resolve to the full ARN (with random suffix) — ECS won't accept
        # anything less specific. boto3 DescribeSecret accepts either a
        # name or a partial ARN, and returns the canonical ARN.
        if existing.startswith("arn:aws:secretsmanager:") and existing.count("-") >= 1:
            existing_arn = existing
        else:
            if sess is None:
                die(
                    "Cannot resolve existing secret ARN — no AWS session "
                    "available. Re-run with credentials configured."
                )
            try:
                resp = sess.client("secretsmanager").describe_secret(SecretId=existing)
                existing_arn = resp["ARN"]
                ok(f"Resolved secret {existing} → {existing_arn}")
            except ClientError as e:
                die(f"Cannot find Secrets Manager secret '{existing}': {e}")
        return {
    ...
    if not non_interactive:
        ...
        # ZSCALER_CLOUD has a sensible default — only prompt if explicitly blanked.
        if cloud in ("",):
...
def cmd_deploy(args: argparse.Namespace) -> None:
    ...
    # in-place update by default. Operator can opt out with --fresh to
    # force the "first deploy" prompt sequence (used when the prior stack
    # was destroyed out-of-band and the state file is stale).
    ...
    if existing_state:
        ...
        if partial:
            warn(
                ...
                "as defaults — but check the CFN console first if the "
                "previous stack is in a CREATE_FAILED / ROLLBACK state."
            )
        else:
            ok(
    ...
    # ── 2. Resolve credentials, network, TLS, auth ─────────────────────
    step("2/6  Credentials & networking")
    creds = resolve_credentials(env, args.non_interactive, sess=sess)
    ...
    flags = {
        ...
        "DesiredCount": str(env.get("AWS_DESIRED_COUNT", "1")),
        "TaskCpu": env.get("AWS_TASK_CPU", "512"),
        "TaskMemory": env.get("AWS_TASK_MEMORY", "1024"),
    ...
    print(f"  Credentials:    {creds['CredentialSource']}"
          + (f" ({creds.get('ExistingSecretArn')})" if creds["CredentialSource"] == "UseExisting" else ""))
    ...
    print(f"  Desired count:  {flags['DesiredCount']}")
    print(f"  Task size:      {flags['TaskCpu']} CPU / {flags['TaskMemory']} MiB")
    ...
    if existing is None:
        ...
        cfn.create_stack(
            ...
            Tags=[
                ...
                {"Key": "Deployment", "Value": "ecs-fargate"},
            ],
        )
        operation = "create"
    ...
    # 30+ minutes and the user might Ctrl-C, the script may crash, or the
    # stack may end up CREATE_FAILED. In any of those cases, `destroy`
    # needs to know the stack name and region to clean up.
    log_group_name = f"/aws/ecs/{prefix}"
    if operation != "noop":
        save_partial_state(
            {
    ...
    if operation != "noop":
        ...
        if final == "STACK_DELETED" or not final.endswith("_COMPLETE") or "ROLLBACK" in final:
            ...
            # Try to dump container stdout/stderr from CloudWatch. Most
            # CREATE_FAILED outcomes here are EssentialContainerExited
            # (the application crashed at startup); the only way to know
            # WHY is the Python stack trace inside CloudWatch, so we
            # surface it inline instead of making the operator hunt for
            # it in the console.
            _dump_recent_container_logs(sess, log_group_name)
            console_url = (
    ...
    # stamp it as complete via `finalize_state`. Loading the existing
    # state preserves keys written during the partial phase (e.g. the
    # stack name, region, credential source) and lets us add the
    # post-deploy info on top.
    state = load_state()
    state.update(
        {
            "stack_name": stack_name,
...
def _print_deploy_summary(
    ...
    print(_ansi("1;32", "  ✓  Zscaler MCP Server deployed on AWS ECS-Fargate"))
    print(_ansi("1;32", bar))
    print()
    print(f"  Stack name:     {stack_name}")
    print(f"  Region:         {region}")
    print(f"  Cluster:        {outputs.get('ClusterName', '(unknown)')}")
    print(f"  Service:        {outputs.get('ServiceName', '(unknown)')}")
    ...
    if outputs.get("SecretArn"):
        print(f"  Secret:         {outputs['SecretArn']}")
    print()
    ...
    print(_ansi("1;36", "  ──────────"))
    print("    • Tail logs:    python ecs_fargate_mcp_operations.py logs")
    print("    • Stack status: python ecs_fargate_mcp_operations.py status")
    print("    • Tear down:    python ecs_fargate_mcp_operations.py destroy")
    print()
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    if outputs:
        ...
        for k, v in outputs.items():
            print(f"  {k}: {v}")
    # Live ECS service health
    if outputs.get("ClusterName") and outputs.get("ServiceName"):
        try:
            ecs = sess.client("ecs")
            ...
            if r["services"]:
                svc = r["services"][0]
                print()
                print(f"  ECS desired: {svc['desiredCount']}  running: {svc['runningCount']}  pending: {svc['pendingCount']}")
        except ClientError as e:
            warn(f"Could not query ECS service: {e}")


def cmd_logs(args: argparse.Namespace) -> None:
    """Tail the ECS CloudWatch log group using boto3.

    Implemented in pure boto3 (no shell-out to ``aws logs tail``) so the
    command works regardless of which AWS CLI version is installed on the
    operator's machine — ``aws logs tail`` is an AWS CLI v2 subcommand and
    is missing in v1. boto3 is already a hard dependency of this script,
    so this path has zero new requirements.
    """
    state = load_state()
    if not state:
        die("No state file. Run `deploy` first.")
    ...
    info(
        f"Reading {log_group} (region={region}, since={since_min}m)"
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if code in ("ResourceNotFoundException",):
                    warn(
                        ...
                        "Either the task hasn't logged anything, or the stack "
            ...
            if events:
                latest = _emit(events)
                if latest:
                    # +1ms so we don't re-print the boundary event next round
                    last_ts = latest + 1

            token = resp.get("nextToken")
            if token:
                # More pages of the same window — keep draining
                continue
...
def cmd_destroy(args: argparse.Namespace) -> None:
    ...
    # Short-circuit when the stack is already gone (e.g. previous destroy
    # succeeded but state-file removal failed, or stack was deleted from
    # the console manually). Otherwise delete_stack would raise.
    ...
    if not args.yes and not prompt_bool(
        ...
        "This removes the ALB, ECS service, IAM roles, VPC (if we created it), "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="ecs_fargate_mcp_operations.py",
        description="Deploy & manage the Zscaler MCP Server on AWS ECS-Fargate.",
    )
    ...
    p_status = sub.add_parser("status", help="Show stack + ECS service status.")
    p_status.set_defaults(func=cmd_status)

    p_logs = sub.add_parser(
        "logs",
        help="Tail the ECS CloudWatch log group (boto3, AWS CLI not required).",
    )
    p_logs.add_argument(
