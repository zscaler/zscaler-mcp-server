---
title: Supported Tools Reference
sidebar_label: Supported Tools Reference
# Hide the right-side floating TOC on this page only. Rationale:
#   1. The page already has its own in-page "Table of Contents" right
#      after the intro — duplicating it on the right wastes content
#      width and confuses navigation.
#   2. The auto-generated tables in this page have very long
#      Description cells (the ZIA `zia_get_advanced_settings` row is
#      a single ~80-word paragraph). With the right TOC taking
#      ~250px of horizontal real estate, the tables overflow into
#      the TOC column and render the bottom of the page unreadable.
# The supported-tools page is also the only page where we want full
# content width — every other docs page benefits from the right TOC,
# so this stays a per-page opt-out instead of a site-wide setting.
